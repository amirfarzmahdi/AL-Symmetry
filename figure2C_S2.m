% figure 2C & 2S2: measuring mirror-symmetric viewpoint tuning index in
...
% add path
addpath(genpath('functions')) % set the directory
...
% images setting
figure_names = [{'figure2C'};{'figure2S2A'};{'figure2S2B'}];
nexemplar = 25;
nview = 9;
ncategory = 9;
nplot = 3;
...
for i_layer = 1:length(layers) % number of layers
    ...
    imgs_res_trained_zscored = zscore(imgs_res_trained,2);
    imgs_res_untrained_zscored = zscore(imgs_res_untrained,2);
