function [net_test, lim, Weights, Biases] = fun_Initializeweight(net, ver, stdfac) # adopted from Baek et al., 2021
net_test = net;
net_tmp = net_test.saveobj;

rand_layers_ind = [2, 6, 10, 12, 14, 17, 20, 23];
...
for ind_tl = 1:length(rand_layers_ind)
    ...
    %%% edit
    if ind_tl > 5 % fully connected layers
